...
# dataset & dataloader
from sparse_vit.dataset import load_tiny_imagenet, load_cifar10
...
from sparse_vit.training.utils import validate_device, model_save, freeze_weights
...
def parse_args():
    ...
    parser.add_argument(
        "--cfg_path",
        type=str,
        help="Path to pruning & finetuning configuration `.toml` file.",
    )
...
def main():
    ...
    # Prepare model kws
    build_vit_kw = {}
    build_vit_kw.update(cfg["train"]["model"]["embed"])
    build_vit_kw.update(cfg["train"]["model"]["encoder"])
    build_vit_kw.update(cfg["train"]["model"]["task"])
    build_vit_kw.update(cfg["train"]["model"]["params"])
    ...
    for i, (amount, warmup_epochs, cosine_epochs, lr) in enumerate(
        ...
        model = apply_pruning(
            model=model, amount=amount, prune_method=prune_method, **prune_method_kw
        )
        ...
        if finetune is True:
            ...
            optimizer = optim.AdamW(model.parameters(), lr=lr, **opt_kw)
